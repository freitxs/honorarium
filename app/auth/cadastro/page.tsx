...
export default function CadastroPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    else { const j = await res.json().catch(()=>({})); setError(j.message || "Erro ao cadastrar."); }
  };
    ...
    <div className="container py-12">
      <div className="max-w-md mx-auto bg-card border border-default rounded-2xl p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <div><label className="block mb-1">Nome completo</label><input className="w-full bg-card border border-default rounded-xl px-3 py-2" required value={name} onChange={(e)=>setName(e.target.value)} /></div>
        ...
        <p className="text-muted text-sm mt-4"><a href="/auth/login">Já tenho conta</a></p>
      </div>
    </div>
  );
}
